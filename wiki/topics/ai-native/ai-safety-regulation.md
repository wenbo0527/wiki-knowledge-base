---
title: ai safety regulation
author: 尼克·弗瑞 🕵️
product_domain: PD-TOPIC
doc_type: 其他
tags: [topics, ai-native]
date: 2026-04-24
---

# AI安全/监管

> AI安全研究、对齐问题、地缘政治风险、监管政策

---

## 元信息

- **创建时间**: 2026-04-18
- **更新时间**: 2026-05-20 (已审核，无需更新)
- **类型**: subtopic
- **标签**: #ai-safety #regulation #alignment #geopolitics #security

## 核心问题

1. **AI安全研究** - 对齐研究、自动化对齐、安全边界
2. **地缘政治风险** - 中美AI竞争、出口管制、供应链安全
3. **监管政策** - 各國AI监管框架、合规要求

---

## 关键事件

### Pentagon Paradox

Anthropic同时被五角大楼封杀和白宫推荐，形成矛盾局面。

### Bengio AI自我保存警告

图灵奖得主创办Lawzero，发出最强AI安全警告。

---

## 相关洞察

- [[insights/ai-technology/agent-engineering/insight-20260418-anthropic-capability-stratification]] - Opus 4.7能力分级
- [[insights/ai-technology/agent-engineering/insight-20260418-bengio-ai-self-preservation]] - Bengio警告
- [[insights/ai-technology/ai-safety-regulation/insight-20260418-pentagon-paradox]] - Pentagon Paradox分析

---

*最后更新: 2026-04-18*
*维护者: 尼克·弗瑞*
